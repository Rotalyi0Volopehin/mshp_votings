...
class CommonFields:
    ...
    @staticmethod
    def get_description_field(widget, required, label="Описание"):
        return forms.CharField(widget=widget, label=label, min_length=1, max_length=4096, required=required)
...
class RegistrationForm(forms.Form):
    ...
    password2 = forms.CharField(widget=forms.PasswordInput, label="Повторите пароль", min_length=1, max_length=32,
                                required=True)
...
class RunVotingForm(forms.Form):
    voting_title = CommonFields.get_voting_title_field(True)
    action = forms.ChoiceField(label="Запрос на", widget=forms.RadioSelect(),
                               choices=[(1, "начало"), (2, "завершение")], required=True)
